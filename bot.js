...
async function sendOnboardStep(ctx, tgId) {
  ...
  if (s.step < ONBOARD_STEPS.length) {
    await ctx.reply(ONBOARD_STEPS[s.step].q);
  } else {
    s.perma_step = 0;
    s.perma_scores = {};
    await ctx.reply('Отлично! Теперь пройдём тест PERMA.\n\n5 вопросов по шкале от 1 до 5. Отвечай честно!\n\nПоехали!');
...
async function sendPermaQ(ctx, tgId) {
  ...
  await ctx.reply('Вопрос ' + (s.perma_step + 1) + ' из 5\n\n' + q.title + '\n\n' + q.text, scoreKeyboard());
}
...
bot.start(async (ctx) => {
  ...
  if (payload && payload.startsWith('onboard_')) {
    ...
    await ctx.reply('Привет, ' + ctx.from.first_name + '!\n\nТвой коуч приглашает заполнить карточку участника.\nЭто займёт 3-5 минут. Данные увидит только твой коуч.\n\nПоехали!');
...
bot.on('text', async (ctx) => {
  ...
  if (['point_a_profit', 'debt_biz', 'debt_personal'].includes(step.key)) {
    ...
    if (isNaN(num)) return ctx.reply('Напиши число. Например: 150000');
    s.data[step.key] = num;
  } else {
...
bot.action(/^sc_(\d)$/, async (ctx) => {
  ...
  if (s.type === 'onboard') {
    ...
    s.perma_step++;
    if (s.perma_step < PERMA_Q.length) return sendPermaQ(ctx, tgId);
...
bot.action('my_groups', async (ctx) => {
  ...
  if (coach.is_blocked) return ctx.reply('Доступ заблокирован.');
  ...
  await ctx.editMessageText('Твои десятки (' + groups.length + '):', Markup.inlineKeyboard(buttons));
});
...
bot.command('invite', async (ctx) => {
  ...
  ctx.reply(
    'Заполни карточку участника — это займёт 3-5 минут.\nОтвечай честно, данные видит только коуч.',
...
bot.command('permatest', async (ctx) => {
  ...
  if (ctx.chat.type !== 'private') {
    return ctx.reply('Тест в личных сообщениях:',
